...
import { Leaf, MapPin, User, Moon, Sun } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onOpenProfile, toggleTheme, isDarkMode }) => {
  return (
    <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-50 border-b border-green-100 dark:border-slate-700 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.location.reload()}>
            <div className="bg-green-600 p-2 rounded-lg text-white shadow-lg shadow-green-600/20">
              <Leaf size={24} />
            </div>
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">GreenRoute</h1>
              <p className="text-xs text-green-700 dark:text-green-400 font-medium">Carbon Footprint Planner</p>
            </div>
          </div>
